        ...
        public void applyFix(@NotNull Project project, @NotNull ProblemDescriptor descriptor,
            ...
            if (methodCall != null) {
                if (methodName.equals(methodCall.getMethodExpression().getReferenceName())) {
                    ...
                    if (editor != null) {
                        final PsiElement lastChild = methodCall.getArgumentList();
                        final int offset = lastChild.getTextOffset() + 1;
                        ShowParameterInfoHandler.invoke(project, editor, psiFile, offset, lastChild, false);
            ...
            final Editor editor = FileEditorManager.getInstance(project).getSelectedTextEditor();
            if (editor != null) {
                final PsiElement lastChild = newElement.getLastChild();
                final int offset = lastChild.getTextOffset() + 1;
                ShowParameterInfoHandler.invoke(project, editor, psiFile, offset, lastChild, false);
                editor.getCaretModel().moveToOffset(offset);
            }
